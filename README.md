...
│   │   │   ├── animations/        # BreathOrb, AsciiMoon
...
3. SUBSTANCE CHECKLIST
   └── Record dosage, ingestion time (critical for come-up timing)

4. ACTIVE SESSION
   ├── Come-Up Phase
   │   ├── Intro sequence
   │   ├── Modules with check-in prompts between each
...
- **Style:** All uppercase, wide tracking
...
### Custom Animations
Defined in `index.css`: `fadeIn`, `fadeOut`, `slideUp`, `slideDown`, `breath-idle`, `orb-glow`
