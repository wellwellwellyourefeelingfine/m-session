/**
 * CycleModal Component
 *
 * Full-screen overlay for displaying the generated cycle diagram PNG.
 * Supports pinch-to-zoom, pan when zoomed, share/download export.
 * Follows LifeGraphModal pattern exactly.
 */
...
export default function CycleModal({ isOpen, closing, onClose, imageUrl, imageBlob }) {
  ...
  const handleExport = useCallback(async () => {
    if (!imageBlob) return;
    try {
      const file = new File([imageBlob], 'the-cycle.png', { type: 'image/png' });
      if (navigator.share && navigator.canShare?.({ files: [file] })) {
        await navigator.share({ files: [file], title: 'The Cycle' });
        return;
      }
      // Fallback: download
      ...
      a.href = url;
      a.download = `the-cycle-${new Date().toISOString().slice(0, 10)}.png`;
      ...
      if (err.name !== 'AbortError') {
        console.warn('Cycle export failed:', err);
      }
    }
  }, [imageBlob]);
  ...
  return (
    ...
    >
      ...
      {/* Diagram image with pinch-to-zoom */}
