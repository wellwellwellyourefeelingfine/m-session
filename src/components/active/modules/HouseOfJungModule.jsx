/**
 * HouseOfJungModule Component
 *
 * A 17-phase guided imagery / archetypal descent meditation with:
 * - 123 pre-recorded TTS audio prompts
 * - Fixed duration (~28-30 minutes, silence-tuned)
 * - No silence expansion, no duration picker
 * - Audio-text sync via shared useMeditationPlayback hook
 *
 * Voice + idle-pill wiring mirrors MettaHeartModule: the selected voice flows
 * into generateTimedSequence (driving audioSrc URLs) and into the voice-aware
 * idle-screen duration estimate via estimateMeditationDurationSeconds.
 */
...
export default function HouseOfJungModule({ module, onComplete, onSkip, onProgressUpdate }) {
  const meditation = getMeditationById('house-of-jung');
