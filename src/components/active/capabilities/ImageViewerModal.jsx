 ...
 *
 * Shared full-screen overlay for displaying a generated PNG image.
 * Supports pinch-to-zoom, pan when zoomed, double-tap toggle, share/download export.
 * Generalizes the pattern from LifeGraphModal / CycleModal.
 *
 * UX: X close top-left, export top-right, floating Done button.
 */
...
export default function ImageViewerModal({
  ...
  const handleExport = useCallback(async () => {
    if (!imageBlob) return;
    try {
      const file = new File([imageBlob], `${imageName}.png`, { type: 'image/png' });
      if (navigator.share && navigator.canShare?.({ files: [file] })) {
        await navigator.share({ files: [file], title: imageName });
        return;
      }
      // Fallback: download
      ...
      a.href = url;
      a.download = `${imageName}-${new Date().toISOString().slice(0, 10)}.png`;
...
/** Icon for the right-slot "view image" button */
export function ViewImageIcon() {
  return (
    <svg width="16" height="16" viewBox="0 0 16 16" fill="none" stroke="currentColor" strokeWidth="1.5">
      <rect x="1" y="2" width="14" height="12" rx="1" />
      <circle cx="5" cy="6" r="1.5" />
      <polyline points="14,12 10,8 7,11 5,9 2,12" />
    </svg>
  );
}
