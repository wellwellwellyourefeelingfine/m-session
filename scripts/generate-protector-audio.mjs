 ...
 *
 * Usage:
 *   ELEVENLABS_API_KEY=your_key node scripts/generate-protector-audio.mjs
 *
 * Options:
...
const OUTPUT_DIR = path.join(PROJECT_ROOT, 'public', 'audio', 'meditations', 'protector');

// --- Import prompts from content definition ---
const { protectorDialogueMeditation } = await import('../src/content/meditations/protector-dialogue.js');
const allPrompts = protectorDialogueMeditation.prompts;
...
if (!apiKey && !dryRun) {
  ...
  console.error('Usage: ELEVENLABS_API_KEY=your_key node scripts/generate-protector-audio.mjs');
  process.exit(1);
}
...
async function main() {
  ...
  console.log(`Protector Dialogue Audio Generator`);
  console.log(`==================================`);
