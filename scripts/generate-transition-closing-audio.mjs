/**
 * Generate Closing Reflection Meditation Audio
 *
 * Calls the ElevenLabs TTS API to generate MP3 files for all 38 clips
 * of the transition-closing meditation. Delivered over the Moonrise animation
 * at the midpoint of the Closing Ritual.
 *
 * Clips span 12 sections: arrival, breath, honor, intention, shape, spectrum,
 * continues, unknowing, help, gratitude, close-container, return.
 *
 * Usage:
 *   ELEVENLABS_API_KEY=your_key node scripts/generate-transition-closing-audio.mjs
 *
 * Options:
 ...
 *   --section SEC   Generate only clips from a section (e.g. --section honor)
 */
...
const OUTPUT_DIR = path.join(PROJECT_ROOT, 'public', 'audio', 'meditations', 'transition-closing');

// --- Import clips from content definition ---
const { transitionClosing } = await import('../src/content/meditations/transition-closing.js');

const allClips = transitionClosing.prompts;
...
if (!apiKey && !dryRun) {
  ...
  console.error('Usage: ELEVENLABS_API_KEY=your_key node scripts/generate-transition-closing-audio.mjs');
  process.exit(1);
}
...
const SECTION_PREFIXES = [
  { prefix: 'close-container-', section: 'close-container' },
  { prefix: 'arrival-', section: 'arrival' },
  { prefix: 'breath-', section: 'breath' },
  { prefix: 'honor-', section: 'honor' },
  { prefix: 'intention-', section: 'intention' },
  { prefix: 'shape-', section: 'shape' },
  { prefix: 'spectrum-', section: 'spectrum' },
  { prefix: 'continues-', section: 'continues' },
  { prefix: 'unknowing-', section: 'unknowing' },
  { prefix: 'help-', section: 'help' },
  { prefix: 'gratitude-', section: 'gratitude' },
  { prefix: 'return-', section: 'return' },
];
...
const SECTION_LABELS = {
  ...
  honor: 'Honoring',
  intention: 'Intention',
  shape: 'Shape of the Session',
  spectrum: 'Spectrum of Experience',
  continues: 'The Work Continues',
  unknowing: 'Integration Takes Time',
  help: 'What Helps',
  gratitude: 'Gratitude',
  'close-container': 'Closing the Container',
  return: 'Return',
};
...
async function main() {
  ...
  console.log(`Closing Reflection Audio Generator`);
  console.log(`==================================`);
  ...
  for (const { section, count } of sectionCounts) {
    if (count > 0) {
      console.log(`  ${getSectionLabel(section).padEnd(26)} ${count}`);
    }
  }
