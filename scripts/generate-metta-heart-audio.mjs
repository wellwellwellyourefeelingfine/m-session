/**
 * Generate Metta Heart Meditation Audio
 *
 * Calls the ElevenLabs TTS API to generate MP3 files for all 86 prompts.
 *
 * Usage:
 *   ELEVENLABS_API_KEY=your_key node scripts/generate-metta-heart-audio.mjs
 *   node scripts/generate-metta-heart-audio.mjs --voice rachel --dry-run
 *
 * Options:
...
const { mettaHeartMeditation } = await import('../src/content/meditations/metta-heart.js');
const allPrompts = mettaHeartMeditation.prompts;
...
if (!apiKey && !dryRun) {
  ...
  console.error('Set it in the environment or in .env, then run node scripts/generate-metta-heart-audio.mjs');
  process.exit(1);
}
...
async function main() {
  ...
  console.log('Metta Heart Audio Generator');
  console.log('===========================');
