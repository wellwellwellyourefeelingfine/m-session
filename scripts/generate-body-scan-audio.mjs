/**
 * Generate Body Scan Meditation Audio
 *
 * Calls the ElevenLabs TTS API to generate MP3 files for all 54 prompts.
 *
 * Usage:
...
const { bodyScanMeditation } = await import('../src/content/meditations/body-scan.js');
...
async function main() {
  const prompts = bodyScanMeditation.prompts;
  ...
  let filtered = prompts.map((p, i) => ({ ...p, index: i }));
  if (onlyId) {
  ...
  if (dryRun) {
    ...
    for (const prompt of filtered) {
      ...
      console.log(`[${prompt.index + 1}/${prompts.length}] ${filename}`);
  ...
  for (let i = 0; i < filtered.length; i++) {
    ...
    console.log(`[${prompt.index + 1}/${prompts.length}] Generating ${filename}...`);
