/**
 * Generate House of Jung Meditation Audio
 *
 * Calls the ElevenLabs TTS API to generate MP3 files for all 123 prompts.
 *
 * Usage:
 *   ELEVENLABS_API_KEY=your_key node scripts/generate-house-of-jung-audio.mjs
 *   node scripts/generate-house-of-jung-audio.mjs --voice rachel --dry-run
 *
 * Options:
 ...
 *   --only ID       Generate only a specific prompt by ID (e.g. --only arrival-01)
 */
...
const { houseOfJungMeditation } = await import('../src/content/meditations/house-of-jung.js');
const allPrompts = houseOfJungMeditation.prompts;
...
if (!apiKey && !dryRun) {
  ...
  console.error('Set it in the environment or in .env, then run node scripts/generate-house-of-jung-audio.mjs');
  process.exit(1);
}
...
async function main() {
  ...
  console.log('House of Jung Audio Generator');
  console.log('=============================');
