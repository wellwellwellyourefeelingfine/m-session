 ...
 *   node scripts/generate-leaves-on-a-stream-audio.mjs --voice rachel --dry-run
 *
 * Options:
...
if (!apiKey && !dryRun) {
  ...
  console.error('Set it in the environment or in .env, then run node scripts/generate-leaves-on-a-stream-audio.mjs');
  process.exit(1);
}
