/**
 * Generate Leaves on a Stream Meditation Audio
 *
 * Calls the ElevenLabs TTS API to generate MP3 files for all 21 prompts.
 * "Leaves on a Stream" — ACT cognitive defusion meditation
 *
 * Usage:
 *   ELEVENLABS_API_KEY=your_key node scripts/generate-leaves-on-a-stream-audio.mjs
 *
 * Options:
...
const OUTPUT_DIR = path.join(PROJECT_ROOT, 'public', 'audio', 'meditations', 'leaves-on-a-stream');

// --- Import prompts from content definition ---
const { leavesOnAStreamMeditation } = await import('../src/content/meditations/leaves-on-a-stream.js');
const allPrompts = leavesOnAStreamMeditation.prompts;
...
if (!apiKey && !dryRun) {
  ...
  console.error('Usage: ELEVENLABS_API_KEY=your_key node scripts/generate-leaves-on-a-stream-audio.mjs');
  process.exit(1);
}
...
async function main() {
  ...
  console.log(`Leaves on a Stream Audio Generator`);
  console.log(`===================================`);
  ...
  if (dryRun) {
    ...
    for (const prompt of filtered) {
      ...
      const conditional = prompt.conditional ? ` [conditional: ≥${prompt.conditional.minDuration} min]` : '';
      console.log(`[${prompt.index + 1}/${allPrompts.length}] ${filename}${conditional}`);
