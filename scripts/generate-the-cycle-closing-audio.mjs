/**
 * Generate The Cycle Closing Meditation Audio
 *
 * Calls the ElevenLabs TTS API to generate MP3 files for all 21 prompts.
 * Both modes (solo/couple) share the same audio files; the variation assembly
 * just filters which prompts to include at runtime.
 *
 * Usage:
 *   ELEVENLABS_API_KEY=your_key node scripts/generate-the-cycle-closing-audio.mjs
 *
 * Options:
 ...
 *   --only ID       Generate only a specific prompt by ID (e.g. --only settle-01)
 */
...
// Voice settings tuned for Theo Silk — same as The Deep Dive
...
const OUTPUT_DIR = path.join(PROJECT_ROOT, 'public', 'audio', 'meditations', 'the-cycle-closing');

// --- Import prompts from content definition ---
const { theCycleClosingMeditation } = await import('../src/content/meditations/the-cycle-closing.js');
const allPrompts = theCycleClosingMeditation.prompts;
...
if (!apiKey && !dryRun) {
  ...
  console.error('Usage: ELEVENLABS_API_KEY=your_key node scripts/generate-the-cycle-closing-audio.mjs');
  process.exit(1);
}
...
async function main() {
  ...
  console.log(`The Cycle Closing Meditation Audio Generator`);
  console.log(`=============================================`);
  ...
  if (dryRun) {
    ...
    for (const prompt of filtered) {
      ...
      if (prompt.variationOnly) console.log(`  Mode: ${prompt.variationOnly} only`);
      console.log();
    }
