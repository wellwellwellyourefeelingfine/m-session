/**
 * Generate The Deep Dive Meditation Audio
 *
 * Calls the ElevenLabs TTS API to generate MP3 files for all 41 prompts.
 * Both modes (solo/couple) share the same audio files; the variation assembly
 * just filters which prompts to include at runtime.
 *
 * Usage:
 *   ELEVENLABS_API_KEY=your_key node scripts/generate-the-descent-audio.mjs
 *
 * Options:
...
// Voice settings tuned for Theo Silk — slightly faster for this meditation
...
const OUTPUT_DIR = path.join(PROJECT_ROOT, 'public', 'audio', 'meditations', 'the-descent');

// --- Import prompts from content definition ---
const { theDescentMeditation } = await import('../src/content/meditations/the-descent.js');
const allPrompts = theDescentMeditation.prompts;
...
if (!apiKey && !dryRun) {
  ...
  console.error('Usage: ELEVENLABS_API_KEY=your_key node scripts/generate-the-descent-audio.mjs');
  process.exit(1);
}
...
async function main() {
  ...
  console.log(`The Deep Dive Meditation Audio Generator`);
  console.log(`========================================`);
