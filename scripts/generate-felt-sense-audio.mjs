/**
 * Generate Felt Sense Meditation Audio
 *
 * Calls the ElevenLabs TTS API to generate MP3 files for all 38 prompts.
 * Both variations share the same audio files; the variation assembly
 * just filters which prompts to include at runtime.
 *
 * Usage:
 *   ELEVENLABS_API_KEY=your_key node scripts/generate-felt-sense-audio.mjs
 *
 * Options:
...
const OUTPUT_DIR = path.join(PROJECT_ROOT, 'public', 'audio', 'meditations', 'felt-sense');

// --- Import prompts from content definition ---
const { feltSenseMeditation } = await import('../src/content/meditations/felt-sense.js');
const allPrompts = feltSenseMeditation.prompts;
...
if (!apiKey && !dryRun) {
  ...
  console.error('Usage: ELEVENLABS_API_KEY=your_key node scripts/generate-felt-sense-audio.mjs');
  process.exit(1);
}
...
async function main() {
  ...
  if (dryRun) {
    ...
    for (const prompt of filtered) {
      ...
      if (prompt.variationOnly) console.log(`  Variation: ${prompt.variationOnly} only`);
      console.log();
    }
